---
title: "Practical 1: Atmospheric state and forcings"
date: "2024-10-15"
format:
  html:
    code-fold: true
...
```
| ​          | P: Pressure \[hPa\] | T: Absolute temperature \[K\] | theta: Potential temperature read \[K\] | theta: Potential temperature calculated \[K\] |
|-----------|---------------------|-------------------------------|-----------------------------------------|-----------------------------------------------|
| top, ABL  | ​                    | ​                              | ​                                        | ​                                              |
| Parcel P1 | ​                    | ​                              | ​                                        | ​                                              |
| Parcel P2 | ​                    | ​                              | ​                                        | ​                                              |

: Readings of atmospheric temperature profile @fig-atm_profile {#tbl-atm_profile}
...
with $\text{E}[\psi] = \overline{\psi}$ the averaged value and $\psi'$ the random fluctuating part. In addition with the assumption of no mean veritcal wind velocity ($\bar{w} = 0$, see introdutory slides), it follows that: 
...
where $\rho = 1.2$ kg/m$^3$ is the air density and $\lambda = 2.45e6$ J/kg the latent heat of vaporisation. Hint: use a programming language of your choice to perform the analysis.

<!-- For next year, let the students derive this formula of the covariance themsevles -->

## Description of the changes in large-scale atmospheric forcing

With the introductory analysis performed, it is now time to carry out sensitvity experiments with CLASS. For more information on how to use the graphical user interface of CLASS, see `CLASS_model_guide.pdf`. The model parameters are set to be representative if a site in the mid-latitude (approximately 50°N).
...
@tbl-free_tropo and @fig-cycl_anticycl summarise the change that have occured in the large-scale atmospheric forcing parameters in the free troposphere: ABL jump (inversion) ($\Delta \theta$ \[K\]), free tropospheric lapse rate ($\gamma_{\theta}$ \[K/m\]) and horizontal wind divergence ( $\nabla \cdot \mathbf{u}_h$ \[1/s\]).
...
|---|---|---|---|
| Cyclinic ($C$) | 1.0 | 0.001 | 0 |
| Anti-cyclonic ($AC$) | 4.0 | 0.009 | 1e-5 |

: Observed values for the three large-scale atmospheric forcing parameters for both cyclonic and anti-cyclonic conditions in the free troposphere {#tbl-free_tropo}
...
Before moving to the problems, the three considered governing equations as given in the introductory slides are repated: 
$$
...
By analyzing the evolution of $\Delta \theta$, find at which time step ($t_i$) $\Delta \theta$ starts increasing in the experiments. Describe the differences in the growth rate of $h$ and $\langle \theta \rangle$ before and after this time $t_i$ for both experiments. Use @eq-state_h and @eq-state_theta to support your discussion.

Hint: save plots in the `Graph` interface in order to compare the temporal evolution of ABL variables against each other. Also include the plots later on in your report.
...
Hint: Exploring vertical profiles of $\langle \theta \rangle$ in `Graph` interface may help you find the answer.
...
-   Positivie relationship between A and B: increase in A leads to increase in
-   Negative relationship between A and B: decrease in A leads to increase in B.

Hint: Look at the 3 main state equations to support the arrows you draw.

![Feedback diagram illustrating the effect of large-scale forcing conditions on the evolution of the ABL. In squares, forcing parameters; in circles, time derivates of key state variables.](figures/feedback_diagram.png){#fig-feedback_diagram}
...
Using @eq-state_h and @eq-state_jump to support your discussion, analyze the role of $\gamma_\theta$ in the boundary layer growth and temperature evolution.

Hint: study vertical profiles in \`Graph´ interface for easier interpretation.
